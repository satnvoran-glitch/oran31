...
interface SeriesViewProps {
  ...
  onUpdateServerConfig: (serverId: 'series', data: Partial<XtreamServerConfig>) => Promise<void>;
  onTestConnection: (serverId: 'series', data?: any) => Promise<any>;
}
...
export const SeriesView: React.FC<SeriesViewProps> = ({
  ...
  const loadData = async () => {
    try {
      ...
      const [catRes, seriesRes] = await Promise.all([
        api.getXtreamSeriesCategories().catch(() => ({ success: false, categories: [] })),
        api.getXtreamSeriesItems().catch(() => ({ success: false, series: [] }))
      ]);
  ...
  const handleSyncServer = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      await onUpdateServerConfig('series', {
        host: parsed.host,
      ...
      const res = await onTestConnection('series', {
        host: parsed.host,
        username: parsed.username,
        password: parsed.password
      });
      ...
      if (res.success) {
        ...
        setTestResult({
          success: false,
          error: res.error || 'Impossible de se connecter au serveur Xtream Series.'
        });
      }
    } catch (err: any) {
    ...
    <div className="p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="bg-[#0F0F14] p-6 rounded-2xl border border-gray-800/80 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-xl bg-purple-950/60 border border-purple-500/30 flex items-center justify-center text-purple-400 shadow-inner">
            <Tv className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2.5">
              Series (Serveur Xtream #2)
              <span className="px-2.5 py-0.5 rounded-full text-[10px] uppercase tracking-wider font-extrabold bg-purple-500/10 text-purple-400 border border-purple-500/20">
                Direct Xtream
              </span>
            </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0F0F14] border border-gray-800/80 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-purple-500/30 transition-all">
        ...
        <div className="bg-[#0F0F14] border border-gray-800/80 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-purple-500/30 transition-all">
          ...
          <p className="text-xs text-gray-400 mt-2 flex items-center gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            Séries & épisodes consultables en streaming direct
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSyncServer} className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-gray-900/90 border border-gray-800 rounded-xl pl-10 pr-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 font-mono text-xs transition-colors"
              />
            </div>
          ...
          <div className="pt-2 flex justify-end">
            <button
              type="submit"
              disabled={testing}
              className="px-6 py-3 rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-bold text-xs shadow-lg flex items-center gap-2.5 transition-all"
            >
